...
内置 **62 种语言**。菜单中汉语及其方言在前，其次为中国少数民族语言，再次为周边国家语言，其余按语言代码排列：

| 分组 | 语言 |
|---|---|
| 汉语及方言 | 简体中文、繁體中文、粤语、闽南语、潮汕话、客家话、吴语、文言 |
| 中国少数民族语言 | བོད་ཡིག（藏语）、ئۇيغۇرچە（维吾尔语）、ᠮᠣᠩᠭᠣᠯ ᠪᠢᠴᠢᠭ（蒙古语·传统蒙文）、한국어（朝鲜语/韩语）、Қазақша（哈萨克语） |
| 东亚与东南亚 | 日本語、Tiếng Việt、Bahasa Indonesia、Bahasa Melayu、Filipino、ไทย、မြန်မာ、ភាសាខ្មែរ |
| 南亚 | हिन्दी、বাংলা、اردو、தமிழ்、తెలుగు、मराठी |
| 中东与中亚 | العربية、עברית、فارسی、Türkçe、Oʻzbekcha、Azərbaycanca |
| 非洲 | Kiswahili、Ikinyarwanda、አማርኛ、Hausa、Afrikaans、isiZulu |
| 欧洲 | English、Deutsch、Français、Nederlands、Italiano、Español（含 🇲🇽 🇦🇷）、Português（含 🇧🇷）、Svenska、Norsk bokmål、Dansk、Suomi、Polski、Čeština、Русский、Українська、Српски、Magyar、Română、Български、Ελληνικά |

### 关于中国少数民族语言与方言的几点说明

出于严谨，有必要写清楚几件事，避免误解：

- **满语**暂未收录。满语母语者已不足百人，且传统满文为竖排书写，macOS 的菜单控件不支持竖排渲染，勉强横排会严重影响可读性。传统蒙古文（ᠮᠣᠩᠭᠣᠯ ᠪᠢᠴᠢᠭ）存在同样的竖排限制，目前先以横排形式收录，欢迎熟悉这两种文字排版的朋友指点更好的处理方式。
- **回族通用汉语**，历史上并无独立的回语文字系统（经堂语是阿拉伯语、波斯语词汇与汉语混合的宗教用语，非独立语言），因此以简体中文覆盖。
- **"高山族"是对台湾原住民族群的旧称**，实际包含阿美、泰雅、排湾、布农等十余个族群，各有独立语言，同属南岛语系。将其并作一种语言并不准确；后续计划优先收录使用人口最多的**阿美语**，也欢迎相关族群的朋友直接贡献翻译。
- **各汉语方言**的书面形式差异很大：粤语有成熟的书面粤文传统，闽南语、客家语可用汉字书写但用字尚未完全统一，吴语的书写规范仍在发展中。本项目统一采用简体汉字书写（繁體中文与文言除外），个别用字若不符合当地习惯，欢迎指正。

后加入的语言由 AI 辅助翻译后加入的语言由 AI 辅助翻译、尚未经母语者校对，文件头部已注明。**发现表述不地道，欢迎直接改一行提 PR** —— 这是最容易上手的贡献方式。

**自定义或新增语言**：菜单「语言 → 打开语言文件夹…」会直接在访达中打开目录（并自动放一份英文模板），把 `zhs.template.ini`（简体中文）或 `en.template.ini`（英文）复制一份，改名为目标语言代码（如 `nl.ini`），翻译等号右侧即可，重启后出现在语言菜单里。同名文件优先于内置版本。
