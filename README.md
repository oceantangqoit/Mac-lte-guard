...
**装完这一次，以后不用再管。** 菜单里打开「更新…」，勾上「查到新版本自动装好」并选一个查询间隔，程序此后自己查、自己下、自己装，装完自动重启，全程不弹任何框。它下载用的也是 `curl`，同样不会遇到上面那个提示。

自动更新怎么保证装的是我们发的东西：程序先向 GitHub 官方接口直连查询，接口会同时给出安装包的 SHA-256；包本身可以走镜像加速下载（大陆直连 GitHub 常常不通），但装之前一定要与官方给的校验和逐位比对。**镜像能返回任意内容，可它改不了内容还能对上官方哈希。** 万一校验和对不上，或者连官方接口都没连上、拿不到校验和，程序就不会无人值守安装——只下好、留一条「更新已就绪」，由你自己确认。

> 这是在没有代码签名的前提下能做到的最好程度。彻底的解法是 Apple 的签名与公证（每年 99 美元的开发者账号）。本项目开源免费，暂不打算走这条路；如果你介意，可以自己构建，或者每次装之前手动核对校验和。
